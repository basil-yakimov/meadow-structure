...
md.sp.list <- colnames(dat2)
md.g.list <- sapply(strsplit(md.sp.list, "_"), function(x) paste(x[1]))

id2 <- which(md.sp.list %in% GBOTB.extended$tip.label)
md.sp.list[-id2]
md.sp.list[110] <- "Silene_flos-cuculi"
...
md.f.list[103:105] <- "Polygonaceae"
...
md.tree2 <- phylo.maker(sp.list = md.df, scenarios="S1")
md.tree2 <- md.tree2$scenario.1

save(md.tree2, file = "clean.data/meadows-phylo-angiosp-only.rda")
save(dat2, file = "clean.data/meadows-2018-angiosp-only.rda")
